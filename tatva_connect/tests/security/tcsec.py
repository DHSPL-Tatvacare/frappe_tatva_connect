...
single isolated uv venv, with a rich at-a-glance UI. See tatva_connect/tests/security/README.md for install/usage.
...
REPO_ROOT = Path(__file__).resolve().parents[3]  # tatva_connect/tests/security/tcsec.py -> repo root
...
SEMGREP_RULES = REPO_ROOT / "tatva_connect" / "tests" / "security" / "semgrep-rules"
SECRETS_BASELINE = REPO_ROOT / "tatva_connect" / "tests" / "security" / ".secrets.baseline"
...
from tatva_connect.tests.security import static_checks  # noqa: E402
...
def tool(name: str) -> str:
	...
	if not found:
		sys.exit(f"tcsec: '{name}' not found — install: pip install -r tatva_connect/tests/security/requirements.txt")
	return found
...
def preflight(args) -> list[str]:
	...
	if args.pdf is not None:
		if importlib.util.find_spec("playwright") is None:
			errs.append("--pdf needs playwright — run: pip install -r tatva_connect/tests/security/requirements.txt")
		else:
			err = ensure_chromium()
			if err:
				errs.append(err)
...
def build_parser() -> argparse.ArgumentParser:
	...
	cmd("secrets-baseline", "(re)generate tatva_connect/tests/security/.secrets.baseline")
	return p
