...
consumers read this list and add no checks of their own, so they can never drift:
  ...
  * tatva_connect/tests/security/tcsec.py             — runs each with a rich local UI (optional sugar)
...
APP = "tatva_connect"
SEMGREP_RULES = "tatva_connect/tests/security/semgrep-rules"
SECRETS_BASELINE = "tatva_connect/tests/security/.secrets.baseline"  # pragma: allowlist secret
...
SECRETS_EXCLUDE = r"tatva_connect/tests/"
