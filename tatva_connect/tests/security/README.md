...
```bash
python tatva_connect/tests/security/tcsec.py <command> [flags]
# or, fully-qualified without activating:
~/.venvs/venv-python-frappe-sec/bin/python tatva_connect/tests/security/tcsec.py <command>
```
...
| `semgrep`        | semgrep        | pattern SAST + custom Frappe rules (`tatva_connect/tests/security/semgrep-rules/`) | no          |
...
| `secrets-baseline` | detect-secrets | (re)generate `tatva_connect/tests/security/.secrets.baseline`               | no              |
...
- `--pdf [PATH]` — a branded TatvaCare PDF report rendered with Playwright (banner + summary cards →
  ...
  ```bash
  python tatva_connect/tests/security/tcsec.py static --pdf                    # findings-only PDF, no AI
  python tatva_connect/tests/security/tcsec.py static --pdf --ai-explain        # analysis-first PDF + console analysis
  python tatva_connect/tests/security/tcsec.py all --pdf ~/Desktop/sec.pdf --ai-explain
  ```
...
```bash
python tatva_connect/tests/security/tcsec.py static                   # everything offline, in one shot
python tatva_connect/tests/security/tcsec.py static --ai-explain       # + per-command claude analysis
python tatva_connect/tests/security/tcsec.py all                       # + the bench runtime suite
python tatva_connect/tests/security/tcsec.py sast --app tatva_connect --app api-docs   # scan extra dirs
python tatva_connect/tests/security/tcsec.py deps --in-container        # audit the real bench env (frappe + all)
python tatva_connect/tests/security/tcsec.py static --format json --fail-fast
```
...
```bash
python tatva_connect/tests/security/tcsec.py lint
python tatva_connect/tests/security/tcsec.py sast
python tatva_connect/tests/security/tcsec.py semgrep --semgrep-config tatva_connect/tests/security/semgrep-rules
python tatva_connect/tests/security/tcsec.py secrets
python tatva_connect/tests/security/tcsec.py locks
```
...
Pinned in `tatva_connect/tests/security/requirements.txt` (public repo → reproducible CI). Bump deliberately, then
