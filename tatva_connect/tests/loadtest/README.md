...
docker exec tatvalocal-backend-1 bash -lc \
  'cd /home/frappe/frappe-bench/sites && ../env/bin/python -B \
   /home/frappe/frappe-bench/apps/tatva_connect/tatva_connect/tests/loadtest/preflight.py check'
```

Then pull, load, and replay:

```bash
.venv/bin/python -m tatva_connect.tests.loadtest.pull anaya tatvapractice --leads 250
.venv/bin/python -m tatva_connect.tests.loadtest.run  anaya tatvapractice
.venv/bin/python -m tatva_connect.tests.loadtest.run  anaya tatvapractice --replay
```
