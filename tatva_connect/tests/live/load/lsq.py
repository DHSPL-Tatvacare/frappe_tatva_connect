...
class LSQ:
	def __init__(self, creds, rate_delay=0.4, timeout=90, logger=None):
	...
	def _call(self, endpoint, params=None, body=None, tries=4):
		...
		for attempt in range(tries):
			...
			try:
				...
				if self.logger:
					self.logger.event("lsq", short, 0, (time.perf_counter() - started) * 1000, False, str(exc)[:60])
				if attempt == tries - 1:
			...
			if resp.status_code in (429, 502, 503, 504):
				if self.logger:
					self.logger.event("lsq", short, resp.status_code, ms, False, "throttled/retry")
				if attempt == tries - 1:
			...
			if resp.status_code != 200:
				if self.logger:
					self.logger.event("lsq", short, resp.status_code, ms, False, resp.text[:60])
				raise LSQReadOnlyError(f"{resp.status_code} on {endpoint}: {resp.text[:300]}")
			if self.logger:
				self.logger.event("lsq", short, 200, ms, True, "")
			return resp.json()
