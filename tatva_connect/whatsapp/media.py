...
state: attached to the CRM Lead, private, in Azure, stamped with the WATI message id, and the WhatsApp
...
def ensure_lead_media(lead: str, wati_id: str, filename: str, content: bytes):
	...
	return find_lead_media(lead, wati_id) or file_manager.save(
		content,
		filename=filename,
		attached_to_doctype="CRM Lead",
		attached_to_name=lead,
		meta={"custom_wa_message_id": wati_id},
	)
...
def adopt_outbound_media(file_url: str, lead: str, wati_id: str):
	...
	re-home it onto the lead (private, stamped with the WATI id, blob re-keyed into the lead folder).
	Returns the File's (possibly new) proxy URL. Idempotent."""
	fd = file_manager.find(file_url=file_url)
	if not fd:
		return file_url
	return file_manager.rehome(fd, "CRM Lead", lead, meta={"custom_wa_message_id": wati_id})
