...
// when a template is picked, show its template variables (name + sample) so the operator knows
...
frappe.ui.form.on('WhatsApp Notification', {
  template(frm) { whatsapp_notif_show_vars(frm); },
  refresh(frm) { whatsapp_notif_show_vars(frm); },
});

function whatsapp_notif_show_vars(frm) {
