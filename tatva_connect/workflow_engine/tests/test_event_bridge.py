...
# See license.txt
"""Completing a task wakes the journey that raised it — and only that one.

The engine parks a journey on an outcome and something in the world produces it later. What makes that safe
...
same doctype, the same status, on the same lead, and the journey must stay parked. A suite that only proves
...
class TestEventBridge(FrappeTestCase):
	...
	def _park(self):
		"""Run to the Wait. Returns (journey_name, the task the Create Task node raised)."""
	...
	def _status(self, journey_name):
		return frappe.db.get_value(fx.JOURNEY_DT, journey_name, ["status", "current_node"], as_dict=True)
	...
	def test_the_node_stamps_its_token_on_the_task_it_raised(self):
		...
		self.assertEqual(
			...
			f"{journey_name}::raise",
			"the task must carry the journey and node that raised it",
		)

	def test_the_wait_correlates_on_that_same_token(self):
		journey_name, _raised = self._park()
		row = frappe.db.get_value(
			fx.JOURNEY_DT, journey_name, ["status", "awaiting_signal", "awaiting_correlation"], as_dict=True
		)
		self.assertEqual(row.status, "Parked")
		self.assertEqual(row.awaiting_signal, "task.completed")
		self.assertEqual(row.awaiting_correlation, f"{journey_name}::raise")
	...
	def test_completing_the_task_emits_the_correlated_outcome(self):
		...
		events = frappe.get_all(
			fx.SIGNAL_DT, filters={"subject_name": self.lead.name},
	...
	def test_the_delivered_outcome_resumes_the_run(self):
		...
		signals.resume_for_signal(
			"CRM Lead", self.lead.name, "task.completed", correlation=f"{journey_name}::raise"
		)
		frappe.db.commit()

		row = self._status(journey_name)
	...
	def test_an_unrelated_task_does_not_wake_the_run(self):
		"""Same lead, same doctype, same terminal status — and the journey must NOT move.
		...
		# outcome for another task cannot move this journey.
		from tatva_connect.workflow_engine import signals
		...
		row = self._status(journey_name)
		self.assertEqual(row.status, "Parked", "a task this journey did not raise must never wake it")
	...
	def test_a_task_with_no_token_emits_nothing(self):
		...
		self.assertEqual(
			frappe.db.count(fx.SIGNAL_DT, {"subject_name": self.lead.name}), 0,
