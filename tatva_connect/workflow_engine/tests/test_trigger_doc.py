...
# See license.txt
"""The record that FIRED a run is not the same thing as the record the run is about.

A durable run's subject is always the resolved parent lead — that is what makes effects act on the lead
and what lets the signal detector find the run. But the thing whose save actually started it may be a
...
said so. An Update Field aimed at the triggering Task raised and failed the run. A Create Task's
file-review branch could never fire at all. All three read as "the workflow just doesn't work".

The negative half matters too: a run the lead itself fired, or one started before this was recorded, has
...
class TestTriggerDoc(FrappeTestCase):
	...
	def _run(self, **values):
		...
		if values:
			frappe.db.set_value(fx.RUN_DT, run.name, values)
		return frappe.get_doc(fx.RUN_DT, run.name)

	def test_the_trigger_doc_is_the_record_that_fired_the_run(self):
		"""The headline. A run started by a Task must hand verbs the TASK."""
		task = frappe.get_doc({
	...
	def test_a_run_with_no_recorded_trigger_falls_back_to_the_subject(self):
		"""A lead-fired run, and every run frozen before this column existed. The lead is the honest
	...
	def test_a_deleted_trigger_record_does_not_break_the_run(self):
		"""The task can be gone by the time a parked run resumes. That must degrade to the lead, not
