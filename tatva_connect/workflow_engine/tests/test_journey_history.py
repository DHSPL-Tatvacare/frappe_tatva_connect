...
SCOPING is the one that matters. A journey carries its lead's field values in `state_json` and a step
...
the caller asks for, and a parked journey reports what it is waiting on — including whether the signal
...
class TestJourneyHistory(FrappeTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
	...
	def tearDownClass(cls):
		"""setUpClass commits (a journey must be durable before the entry segment will retry it), so the
	...
	def _run(self, **values):
		...
		if values:
			frappe.db.set_value(fixtures.JOURNEY_DT, run.name, values)
			frappe.db.commit()
	...
	def test_rep_on_another_line_sees_nothing(self):
		...
		with self.assertRaises(frappe.DoesNotExistError):
			history.journeys_for_subject("CRM Lead", self.lead.name)
		with self.assertRaises(frappe.DoesNotExistError):
			history.journey_steps(run)
		with self.assertRaises(frappe.DoesNotExistError):
			history.journey_state(run)

	def test_rep_on_the_line_sees_the_journey(self):
		run = self._run()
		frappe.set_user(REP_A)
		names = [r["journey"] for r in history.journeys_for_subject("CRM Lead", self.lead.name)["journeys"]]
		self.assertIn(run, names)
		self.assertEqual(history.journey_state(run)["journey"], run)
	...
	def test_missing_and_out_of_scope_are_indistinguishable(self):
		...
		with self.assertRaises(frappe.DoesNotExistError) as absent:
			history.journey_state("WF-RUN-DOES-NOT-EXIST")
		self.assertEqual(str(foreign.exception), str(absent.exception))

	def test_stuck_journeys_hides_a_journey_whose_subject_is_unreadable(self):
		"""The operator view drops a journey the caller cannot read the subject of — proven with a subject
		...
		frappe.db.set_value(fixtures.JOURNEY_DT, orphan, "subject_name", "CRM-LEAD-DELETED-ZZZ")
		frappe.db.commit()

		listed = [r["journey"] for r in history.stuck_journeys(limit=100)["journeys"]]
	...
	def test_stuck_journeys_is_not_a_rep_surface(self):
		...
		frappe.set_user(REP_B)
		# rep_b holds no CRM Workflow Journey DocPerm at all — the operator view is gated before it reads.
		with self.assertRaises(frappe.PermissionError):
			history.stuck_journeys()
	...
	def test_steps_come_back_in_execution_order(self):
		...
		frappe.db.commit()
		steps = history.journey_steps(run)["steps"]
	...
	def test_a_caller_cannot_lift_the_limit(self):
		...
		frappe.db.commit()
		self.assertLessEqual(len(history.journey_steps(run, limit=10_000)["steps"]), history.MAX_STEPS)
	...
	def test_a_page_reports_that_more_remain(self):
		...
		for i in range(3):
			self._step(run, f"n{i}", "ok")
		frappe.db.commit()
		page = history.journey_steps(run, limit=2)
		self.assertEqual(len(page["steps"]), 2)
		self.assertTrue(page["has_more"])
		self.assertFalse(history.journey_steps(run, limit=2, start=2)["has_more"])
	...
	def test_a_parked_run_reports_the_signal_it_waits_on(self):
		...
		waiting = history.journey_state(run)["waiting_on"]
		...
		frappe.get_doc({
			"doctype": fixtures.SIGNAL_DT, "subject_doctype": "CRM Lead", "subject_name": self.lead.name,
			"event_name": "task_done", "correlation": "tok-1", "status": "Pending",
		}).insert(ignore_permissions=True)
		frappe.db.commit()
		self.assertTrue(history.journey_state(run)["waiting_on"]["signal_pending"])
	...
	def test_a_finished_run_waits_on_nothing(self):
		self.assertIsNone(history.journey_state(self._run(status="Done"))["waiting_on"])
	...
	def test_the_failure_reason_is_derived_from_the_last_failed_step(self):
		...
		frappe.db.commit()
		failure = history.journey_state(run)["failure"]
	...
	def test_the_list_carries_the_failure_reason_not_only_the_detail_view(self):
		...
		`_failure` has always been derived, but only `journey_state` returned it — so the list could say
	...
	def test_a_journey_that_did_not_fail_carries_no_reason_in_the_list(self):
	...
	def test_only_a_resting_run_is_counted(self):
		"""The whole rule, in one test. A journey only comes to REST when it parks or dies — everywhere else
		the interpreter walks a whole segment in one pass, so a journey is present for milliseconds.
		...
		Every count test pins its own version, because `_run` COMMITS: without it, one test's parked journey is
	...
	def test_a_step_that_failed_is_not_a_run_that_failed(self):
		...
		any journey that happens to have a failed step, or every recovered run reads as broken."""
		run = self._run(status="Done")
		self._step(run, "n1", "failed", detail="the send bounced, the graph took the other edge")
		self._step(run, "n2", "done")
		frappe.db.commit()
		self.assertIsNone(history.journey_state(run)["failure"])
		self.assertFalse(history.journey_state(run)["stuck"])
	...
	def test_stuck_is_failed_or_a_park_with_nothing_to_wake_it(self):
		...
		self.assertTrue(history.journey_state(failed)["stuck"])
		self.assertTrue(history.journey_state(orphaned)["stuck"])
		self.assertFalse(history.journey_state(on_a_clock)["stuck"])
		self.assertFalse(history.journey_state(on_a_signal)["stuck"], "a named signal can still arrive")
		self.assertFalse(history.journey_state(running)["stuck"])
	...
	def test_nothing_here_writes(self):
		...
		frappe.db.commit()
		before = frappe.db.get_value(fixtures.JOURNEY_DT, run, ["status", "modified", "current_node"], as_dict=True)
		history.journey_state(run)
		history.journey_steps(run)
		history.journeys_for_subject("CRM Lead", self.lead.name)
		history.stuck_journeys()
		after = frappe.db.get_value(fixtures.JOURNEY_DT, run, ["status", "modified", "current_node"], as_dict=True)
