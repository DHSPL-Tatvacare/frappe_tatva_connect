...
touches an Journey. It then OPTIMISTICALLY enqueues `resume_for_signal` (enqueue-after-commit,
...
(Redis flush, worker death) costs only latency - the reconciler sweep re-drives the parked Journey from
the row it can see (`wakeups.reconciler_sweep`). An early delivery (before the Journey parks) simply
waits in the inbox and is consumed the moment the interpreter reaches the Wait - the wakeup cannot drop.

`resume_for_signal` claims the matching `Parked` Journey `for_update`, re-checks status (F6), and
re-enters `advance`, which consumes the inbox row via `_consume_signal`. Idempotent: an Journey that is
...
def resume_for_signal(subject_doctype, subject_name, signal_name, correlation=None):
	...
	name = frappe.db.get_value(JOURNEY_DT, filters, "name", for_update=True)
	if not name:
