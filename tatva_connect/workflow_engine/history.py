...
# See license.txt
"""Journey history — the read surface over what the engine already wrote.

WHY THIS EXISTS
---------------
When a lead does not get her task or her message, nobody can answer why. The engine already
records everything the answer needs: `CRM Workflow Journey` says where a journey sits and what it is
...
IT STORES NOTHING
-----------------
Every value here is read off the journey row or DERIVED from the step log at answer time. There is no
summary column, no step counter and no second status, because any of those could drift from the
log it claims to summarise. A failed journey's reason is the detail of its last `failed` step — the
row `interpreter._fail` wrote — not a copy of it. "Stuck" is a predicate over the journey's own park
...
`journeys_for_subject` to avoid an N+1 on the page that loads first — and that was reversed on
...
`_failure` returns None on every status but `Failed` before it touches the log, so a page of journeys
asks once per FAILED journey, not once per journey.

THE GATE
--------
Journey history is patient data — `state_json` holds the lead's field values and a step `detail` can
...
The rule is asked here rather than left to the `CRM Workflow Journey` DocPerm on purpose. The doctype
...
def journeys_for_subject(subject_doctype, subject_name, limit=20, start=0):
	...
	# a `CRM Workflow Journey` DocPerm the rep whose lead this is deliberately does not have.
...
@frappe.whitelist()
def journey_steps(journey, limit=200, start=0):
	"""What happened, in order: every node this journey executed, its outcome, its detail, its duration."""
	_readable_journey(journey)
	...
	rows = frappe.get_all(  # authz-ok: tier-b — gated by _readable_journey() on this exact journey, above
		STEP_LOG_DT,
...
@frappe.whitelist()
def journey_state(journey):
	"""Why this journey stopped — parked on what and until when, failed for what reason, or done."""
...
@frappe.whitelist()
def stuck_journeys(limit=25, start=0, workflow=None):
	"""Journeys nobody will come back to, newest activity first, so an operator sees them unprompted.

	Two disjoint sets by construction — a journey is Failed or it is Parked, never both — so they are two
	filters rather than one predicate that would have to re-derive the disjunction in SQL.
	"""
	if not frappe.has_permission(JOURNEY_DT, "read"):
...
def node_counts(workflow, workflow_version=None):
	...
	for row in frappe.get_list(
		...
		if not row.current_node:
			continue  # a journey that died before it reached a node has nowhere to be counted
...
def _summary(row):
	"""One journey as the UI needs it: its own columns, plus the two things that are only ever derived."""
	return {
		"journey": row.name,
		...
		"stuck": _is_stuck(row),
		# The third derived answer; costs a query only for a journey that really failed.
		"failure": _failure(row),
	}


def _waiting_on(row):
	"""What a parked journey is waiting for — the three park columns `interpreter._park` wrote and the
	sweep wakes it by, plus whether a signal it named is actually buffered.

	`None` for a journey that is not parked: a finished journey waits on nothing, and reporting its last
...
def _signal_pending(row):
	...
	return bool(frappe.db.exists(SIGNAL_DT, filters))


def _failure(row):
	"""Why a failed journey failed, DERIVED: the last `failed` step of its log, which is the row
	`interpreter._fail` wrote. Nothing is copied onto the journey, so this can never disagree with the log
	it is read from. `None` for a journey that has not failed."""
	if row.status != "Failed":
		return None
	rows = frappe.get_all(
		STEP_LOG_DT,
		filters={"journey": row.name, "outcome": "failed"},
...
def _is_stuck(row):
	"""A journey nobody will come back to: Failed (terminal by construction — `_fail` drops `active_key`
...
def _readable_journey(journey):
	"""One journey's columns, after gating on the record it is about. The row is read permission-free on
	purpose — it is read in order to find out WHICH subject to authorise, and the next line is the
	authorisation. A journey that does not exist and a journey on another rep's line throw identically."""
	row = frappe.db.get_value(JOURNEY_DT, journey, _JOURNEY_FIELDS, as_dict=True) if journey else None
	if not row:
		frappe.throw(_("Not found"), frappe.DoesNotExistError)
...
def _bounded(value, ceiling):
	...
	with 500 journeys must not be able to ask for all of them, nor for all of their steps."""
	try:
		size = int(value)
