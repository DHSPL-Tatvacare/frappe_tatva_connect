...
single isolated uv venv, with a rich at-a-glance UI. See tatva_connect/security/README.md for install/usage.
...
REPO_ROOT = Path(__file__).resolve().parents[2]  # tatva_connect/security/tcsec.py -> repo root
# sys.prefix is the active venv root; its bin/ holds the console scripts. (Do NOT derive this from
# sys.executable + .resolve() — a venv python is a symlink back to the base install,
# whose bin/ lacks the venv-only tools.)
VENV_BIN = Path(sys.prefix) / "bin"
SEMGREP_RULES = REPO_ROOT / "tatva_connect" / "security" / "semgrep-rules"
SECRETS_BASELINE = REPO_ROOT / "tatva_connect" / "security" / ".secrets.baseline"
...
# The static gate is defined ONCE in static_checks.py (this same folder); tcsec is sugar OVER it, so
# `tcsec static` and `pytest tatva_connect/tests/static` can never run a different set of checks.
sys.path.insert(0, str(REPO_ROOT))
from tatva_connect.security import static_checks  # noqa: E402

_SPINE: dict[str, dict] = {}  # cached argv-by-name per outdir (git ls-files / declared-deps once)
...
def tool(name: str) -> str:
	...
	if not found:
		sys.exit(f"tcsec: '{name}' not found — install: pip install -r tatva_connect/security/requirements.txt")
	return found
...
def _spine_argv(name: str, outdir: Path) -> list[str]:
	"""The argv for one static scanner, from the SINGLE source of truth (static_checks) — the SAME
	list the pytest static lane runs, so tcsec can never check a different set. Resolved once per
	outdir (git ls-files + declared-deps computed a single time)."""
	key = str(outdir)
	if key not in _SPINE:
		outdir.mkdir(parents=True, exist_ok=True)
		_SPINE[key] = dict(static_checks.scanner_checks(REPO_ROOT, tool=tool, workdir=str(outdir)))
	return list(_SPINE[key][name])


def step_lint(args, outdir: Path) -> dict:
	return run("lint", _spine_argv("lint", outdir), outdir, args)


def step_sast(args, outdir: Path) -> dict:
	argv = _spine_argv("sast", outdir)
	if args.format == "json":
		argv += ["-f", "json", "-o", str(outdir / "bandit.json")]
	return run("sast", argv, outdir, args)


def step_deps(args, outdir: Path) -> dict:
	if args.in_container:  # local-only extra: audit the LIVE bench env instead of the pinned deps
		...
		argv = [tool("pip-audit"), "-r", str(reqs)]
	else:
		argv = _spine_argv("deps", outdir)
	if args.format == "json":
		argv += ["-f", "json", "-o", str(outdir / "pip-audit.json")]
	return run("deps", argv, outdir, args)
...
def step_secrets(args, outdir: Path) -> dict:
	...
	return run("secrets", _spine_argv("secrets", outdir), outdir, args)


def step_semgrep(args, outdir: Path) -> dict:
	argv = _spine_argv("semgrep", outdir)
	if args.format == "json":
		argv += ["--json", "--output", str(outdir / "semgrep.json")]
	return run("semgrep", argv, outdir, args)
...
def step_secrets_baseline(args, outdir: Path) -> int:
	"""Generate/refresh tatva_connect/security/.secrets.baseline from the tracked tree."""
...
def preflight(args) -> list[str]:
	...
	if args.pdf is not None:
		if importlib.util.find_spec("playwright") is None:
			errs.append("--pdf needs playwright — run: pip install -r tatva_connect/security/requirements.txt")
		else:
			err = ensure_chromium()
			if err:
				errs.append(err)
...
def build_parser() -> argparse.ArgumentParser:
	...
	cmd("secrets-baseline", "(re)generate tatva_connect/security/.secrets.baseline")
	return p
