...
carrying the SAME shape the interpreter mints: `{run_name}::{node_id}` (`interpreter._run_verb`). This
module is the ONE place that reads that stamp back into a human label + the run to deep-link to. It never
...
# doctype -> the column the engine stamps its `{run_name}::{node_id}` token on. Seeded with the two that
...
def automation_origin(doctype, name):
	"""`{"label", "run"}` when this row carries its declared workflow stamp, else `None`.

	The stamp is `{run_name}::{node_id}`; the run_name is everything left of the FIRST `::`, resolved to its
	workflow's label off the `CRM Workflow Run` row. A stamp without `::` (malformed / not ours) or a run
	...
	if not stamp or "::" not in stamp:
		return None
	run_name = stamp.split("::")[0]
	label = frappe.db.get_value("CRM Workflow Run", run_name, "workflow")
	if not label:
		return None
	return {"label": label, "run": run_name}
