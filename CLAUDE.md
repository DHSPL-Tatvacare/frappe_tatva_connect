...
13. **Embedding a popover-based control inside a frappe-ui `Dialog` needs `disableOutsideClickToClose`.** `Filter`/`ColumnSettings`/`SortBy` (and any Popover) teleport their body OUTSIDE the dialog panel, so interacting with them fires reka-ui `interact-outside` and closes the dialog. Set `disableOutsideClickToClose` on the host Dialog.
14. **One remount source only — never stack `:key`s.** `App.vue` keys `<router-view>` on `$route.fullPath`, so a route-param-driven view already remounts the whole page on any change. An additional `:key` on a child (e.g. the list keyed on the active view) double-mounts → **double API fetch** — visible when an async step (`await store…reload()`) splits the route change across ticks. Let the page remount be the single refresh trigger.
