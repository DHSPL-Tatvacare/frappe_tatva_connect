...
2. **Single source of truth = this repo. CODE ONLY IN THE REPO — NEVER TOUCH A BENCH FOLDER WITHOUT EXPLICIT PERMISSION.** All coding — for `tatva_connect` AND the CRM fork — happens in the proper git repo clone (this repo, or a clean clone of `DHSPL-Tatvacare/frappe_tatva_crm`). The local bench (`frappedev-frappe-1`, `apps/*`) is a disposable *copy* used to **run/verify** only; changes take effect there only after you sync them in. **NEVER edit, commit, push, reset, checkout, stash, or otherwise mutate any bench app folder for coding purposes without the user's explicit, per-action say-so** — not even "just to test." If the repo clone is broken/missing, STOP and tell the user; re-clone the repo — do not silently fall back to working out of the bench. Read-only inspection of the bench to diagnose runtime is fine; mutating it is not.
...
18. **`import frappe` first — never hand-roll what the framework ships.** For every DBA/ORM/HTTP/util operation, reach for the native Frappe helper before writing Python/SQL by hand. The helper reads types, parameterizes input, logs, and survives version drift; the hand-rolled version rots. **Default-to-native table (use the left, not the right):**
    - **Schema/DDL:** `frappe.db.rename_column` / `frappe.model.rename_field` · `frappe.db.add_index` / `has_index` · `frappe.db.has_column` — **never** a hand-written `ALTER … CHANGE`/`ADD INDEX` or an `information_schema` `_column_exists` probe (these hardcode the column type and drift across CRM versions).
    - **Reads:** `frappe.db.get_value` / `get_all` / `get_single_value` / `count` / `exists` — **never** a raw `SELECT`/`SELECT COUNT(*)`/`get_value("Singles", …)`. Raw `frappe.db.sql` is allowed **only** for a genuine join/aggregate/`INSERT … ON DUPLICATE KEY` the ORM cannot express, or DDL that must run before model-sync — and it must say so in a comment.
    - **Writes/records:** `frappe.db.set_value`, `frappe.get_doc(...).insert()`, `frappe.new_doc`, `frappe.rename_doc` — never a hand-built `INSERT`/`UPDATE` for ordinary records.
    - **HTTP:** `frappe.integrations.utils.make_get_request` / `make_post_request` — **never** bare `requests`/`urllib`, *except* genuine multipart upload or raw-bytes/streaming responses the wrappers can't carry (comment why).
    - **JSON:** `frappe.parse_json` / `frappe.as_json` — never stdlib `json.loads`/`json.dumps` (parse_json is null/double-parse safe, drops the `try/except` and `or "{}"` crutches).
    - **Dates/casts:** `frappe.utils.now_datetime` / `get_datetime` / `getdate` / `add_to_date` / `cint` / `flt` / `cstr` — never `datetime.now()` or `strptime` for app data (raw epoch `time.time()` for a token-bucket is fine).
...
19. **MOBILE-FRIENDLY IS NON-NEGOTIABLE — CODE RED. Every CRM-fork change ships mobile-correct in the SAME commit, or it does not ship.** The field reps are mobile-first (PWA + phone browser); a desktop-only change is a regression, not a half-done feature. This is a release gate: shipping a layout that breaks at ~390px is a **code-red violation**, same severity as leaking a secret or breaking a fail-closed guard.
    - **Verify at ~390px on EVERY UI change** (lead detail, lists, modals, cards, toolbars, tabs) — desktop-looks-fine is not "done". A change is incomplete until it's proven on a narrow viewport (post-cache-bust per C.9), not just desktop.
    - **The mobile failure modes that keep recurring, and their fixes:** a flex row with a `truncate` child that doesn't shrink ⇒ add **`min-w-0`** on the row AND the grid/flex cell (the #1 card-overflow bug); a chip/badge/attachment row that overflows ⇒ **`flex-wrap gap-2`**; horizontal padding ⇒ match the native timeline (`px-3 sm:px-10`) and gap (`gap-2 sm:gap-4`), never a fixed desktop pad; a horizontally-scrollable strip that resets on remount ⇒ **scroll the active item back into view** (`scrollIntoView({inline:'nearest'})`), never leave the user re-scrolling. Mirror how the **native Calls/Comments timeline** already does it — it is the reference for mobile-correct.
    - **New mobile UX must be reachable on mobile.** A feature gated behind a flag/setting needs a mobile-reachable way to exercise it (URL param, visible control) — never console-only, because a phone has no console. Shipping a mobile feature you can only enable on desktop is the same violation.
    - **Two-panel/builder layouts MUST stack** (`grid-cols-1 sm:grid-cols-2`); popover controls belong on a list toolbar, not inside a modal (C.13). When unsure how a surface should behave on mobile, copy the nearest native primitive that already works there — do not invent a desktop-only layout and "fix mobile later".
